...
$sql="SELECT * FROM cpas_niveau_etudes order by libelle_F;";
...
$tab_niveau_etudes=fn_ResultToArray($result,"id_niveau");
...
$nomTable="array_diplome";

$initialisation="$".$nomTable."=array(); \n";	

//$outStr.="$".$nomTable."[0]['F']='---'; \n";
//$outStr.="$".$nomTable."[0]['N']='---'; \n";
foreach ($tab_niveau_etudes as $key => $value)
{
	
	$outStr.="$".$nomTable."[".$value['id_niveau']."]['F']=\"".$value['libelle_F']."\"; \n";

	$outStr.="$".$nomTable."[".$value['id_niveau']."]['N']=\"".$value['libelle_N']."\"; \n";
