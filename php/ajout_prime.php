...
include('../connect_db.php');
	...
	$sql="
			...
			,'".$id_agent."'
			,'".$id_type_prime."'
			,'".transformDate($date_octroi)."'
			,'".transformDate($date_cloture)."'
			,'".$id_grade."'
			,'".transformDate($date_echeance_code)."'
			,'".transformDate($date_echeance_biennale)."'
			,'".$actif."'
			,NOW()
			,'".$session_username."'
			);
	...
	if(!$result)
	{
		echo "alert('Problème d\'ajout de la prime');";
		exit;
	}
	else
	{
		echo "alert('Nouvelle prime ajoutée avec succès');";
	}
