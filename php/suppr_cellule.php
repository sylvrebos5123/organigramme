...
include('../connect_db.php');
	...
	where 
	id_cel=".$id_cel." and (actif=1 or (actif=0 and date_debut_service > CURDATE()));
	"; */
	...
	where 
	cpas_mouvements_services.id_cel=".$id_cel." and (cpas_contrats.actif=1 or (cpas_contrats.actif=0 and date_debut_service > CURDATE())) and cpas_mouvements_services.statut='N';
	";
	...
	if($tab_result['nbr_agents'] > 0)
	{
		...
		$msg="Des mouvements de services (récents ou prévus dans le futur) existent encore pour cette cellule. Impossible de supprimer cette cellule.";
		echo "alert('$msg');";
		exit;
	}
	...
	if(!$result)
	{
		echo "alert('Problème de suppression du cellule');";
		exit;
	}
	else
	{
		echo "alert('Suppression de cellule réussie');";
	}
	mysqli_close($lien);
